...
class Bounds3
{
  public:
    ...
    inline std::pair<bool, float> IntersectPGetT(const Ray& ray, const Vector3f& invDir,
                           const std::array<int, 3>& dirisNeg) const;

    inline double area() const {
        if(pMax.x > pMin.x && pMax.y > pMin.y && pMax.z > pMin.z)
...
inline std::pair<bool, float> Bounds3::IntersectPGetT(const Ray& ray, const Vector3f& invDir,
                                const std::array<int, 3>& dirIsNeg) const
{
    // invDir: ray direction(x,y,z), invDir=(1.0/x,1.0/y,1.0/z), use this because Multiply is faster that Division
    // dirIsNeg: ray direction(x,y,z), dirIsNeg=[int(x>0),int(y>0),int(z>0)], use this to simplify your logic
    ...
    float tenter = std::max(v0.x, std::max(v0.y, v0.z));
    float texit = std::min(v1.x, std::min(v1.y, v1.z));
    return std::make_pair(tenter < texit && texit >= 0, tenter);
}
